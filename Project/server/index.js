...
app.post("http://localhost:3001/check/ref",(req,res)=>{
    const referal=req.body.referal
    sql = "select * from admin where referal = ?;";
    db.query(sql,[referal],(err,result)=>{
...
app.post("/customer/signup/",(req,res)=>{
    ...
    db.query(sqlInsert1,[fname,lname,password,'abc123','abc123'],(err,result)=>{
        var id2;
...
app.post("/get/bill",(req,res)=>{
    ...
    sql = "select * from billing_info where id = (select max(id) from billing_info);";
...
app.post("/add/cart",(req,res)=>{
    ...
    const sqlcr="update cart set total_price=total_price+? , num_of_products=num_of_products+1 where id=(select max(id) from cart);";
    db.query(sqlcr,[price],(err,result)=>{
        if(err)
        {
            console.log(err);
        }
...
app.get("/product/category",(req,res)=>{
    const catquery='select name from category;'
